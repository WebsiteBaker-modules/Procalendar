...
if (!function_exists('make_dir')){require(WB_PATH.'/framework/functions.php');}
    ...
    function replaceUrl($sTmp){
        ...
        $aPatterns = array(
            ...
            '/([^>]*\s*)('.preg_quote($sMediaUrl, '/').')/siU',
            '/([^>]*\s*)('.preg_quote($sAppUrl, '/').')/siU',
        );
        $aReplacements = array(
            '\1'.$sDocumentRootUrl.'/\3',
            '$1{SYSVAR:MEDIA_REL}$3',
            '$1{SYSVAR:AppUrl.MediaDir}$3',  //  MEDIA_REL
            '$1{SYSVAR:AppUrl}$3',
        );
        $sTmp =  preg_replace($aPatterns, $aReplacements, $sTmp);
        return $sTmp;
   }
...
if (isset($deleteaction)) {
    ...
    $short = $admin->StripCodeFromText($admin->get_post('short'));
//    $long  = $admin->StripCodeFromText($admin->get_post('long'));
    $short = replaceUrl($short);
    ...
    function checkimage($checkname, $custom)
    {
        ...
        if (isset($_FILES[$checkname]['tmp_name']) && $_FILES[$checkname]['tmp_name'] != '')
        {
            ...
            return replaceUrl($st_filename);
        }
        $custom = replaceUrl($custom);
        return $custom;
    }

    if ($usecustom1 <> 0) {$custom1 = $admin->get_post('custom1');}
    if ($usecustom1 == 4) {$custom1 = checkimage('custom_image1', $custom1);}
    if ($usecustom2 <> 0) {$custom2 = $admin->get_post('custom2');}
    if ($usecustom2 == 4) {$custom2 = checkimage('custom_image2', $custom2);}
    if ($usecustom3 <> 0) {$custom3 = $admin->get_post('custom3');}
    if ($usecustom3 == 4) {$custom3 = checkimage('custom_image3', $custom3);}
    if ($usecustom4 <> 0) {$custom4 = $admin->get_post('custom4');}
    if ($usecustom4 == 4) {$custom4 = checkimage('custom_image4', $custom4);}
    if ($usecustom5 <> 0) {$custom5 = $admin->get_post('custom5');}
    if ($usecustom5 == 4) {$custom5 = checkimage('custom_image5', $custom5);}
    if ($usecustom6 <> 0) {$custom6 = $admin->get_post('custom6');}
    if ($usecustom6 == 4) {$custom6 = checkimage('custom_image6', $custom6);}
    if ($usecustom7 <> 0) {$custom7 = $admin->get_post('custom7');}
    if ($usecustom7 == 4) {$custom7 = checkimage('custom_image7', $custom7);}
    if ($usecustom8 <> 0) {$custom8 = $admin->get_post('custom8');}
    if ($usecustom8 == 4) {$custom8 = checkimage('custom_image8', $custom8);}
    if ($usecustom9 <> 0) {$custom9 = $admin->get_post('custom9');}
    ...
    if (trim($name) != "") {
        ...
        $sql .= '`name`      =\''.$database->escapeString($name) . '\', '
              ...
              . 'rec_exclude =\''.$rec_exclude . '\' ';
        $js_back = ADMIN_URL . '/pages/modify.php?page_id='.$page_id . '&month='.$js_start_month . '&year='.$js_start_year . '';
        if (!$database->query($sql . $sqlWhere)) {
            if ($database->is_error()) {
                  $admin->print_error($database->get_error(), $js_back);
            } else {
                $admin->print_success($MESSAGE['PAGES_NOT_SAVED'], $js_back);
            }
        } else {
